...
{
    // Determine the first day of the week for the given date
    QDate firstDayOfWeek = date.addDays(-(date.dayOfWeek() - 1));

    // Custom painting code for the entire week's background
    if (date.dayOfWeek() == 1) { // Only paint once per week, on Monday
        painter->save();
        painter->setBrush(QColor("#3afefe")); // Set your desired color here
        painter->setPen(Qt::NoPen);
        QRect weekRect = rect;
        weekRect.setWidth(rect.width() * 10); // Adjust the rectangle to cover the entire week
        painter->drawRect(weekRect);
        painter->restore();
    }
    ...
    if (date == QDate::currentDate()) {
        painter->save();
        painter->setBrush(QColor("#2f6b7c")); // Light blue color for current date background
        painter->setPen(Qt::NoPen);
...
void CustomCalendarWidget::handleEventDeleted(const QString &eventTitle)
{
    ...
    if (events.contains(date)) {
        ...
        if (events[date].isEmpty()) {
            ...
            QSqlQuery query(db);
            query.prepare("DELETE FROM calendar WHERE event = :event AND date = :date");
            query.bindValue(":event", eventTitle);
            query.bindValue(":date", date.toString(Qt::ISODate));
            if (!query.exec()) {
                QMessageBox::critical(this, "Database Error", query.lastError().text());
...
void CustomCalendarWidget::updateCell(const QDate &date)
{
    // Custom logic to update a specific cell in the calendar
    // This could involve repainting the cell or updating its data
    update(); // Or call a specific method to update the cell if needed
}
